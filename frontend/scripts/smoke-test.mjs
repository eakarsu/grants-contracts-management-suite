const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4730';
...
const adminCookie = await login('admin@grants-contracts.local', 'admin123');
const managerCookie = await login('manager@grants-contracts.local', 'manager123');
const analystCookie = await login('analyst@grants-contracts.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/grants', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/grants', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/grants/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/grants/approve`, {
  method: 'POST',
  headers: {
